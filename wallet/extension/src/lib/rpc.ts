...
export async function fetchCircleAsset(circleId: string, path: string): Promise<{ contentType: string; bodyB64: string }> {
  const result = await rpcCall('circle_asset', [circleId, path], { fast: true }) as Record<string, unknown>;
  return {
    contentType: String(result.content_type ?? 'application/octet-stream'),
    bodyB64: String(result.body_b64 ?? ''),
  };
}
