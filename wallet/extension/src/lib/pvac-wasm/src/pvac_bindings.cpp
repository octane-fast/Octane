...
// Add two ciphers homomorphically
EMSCRIPTEN_KEEPALIVE
uint8_t* pvac_wasm_ct_add(const uint8_t* a_data, int a_len,
                           const uint8_t* b_data, int b_len,
                           int* out_len) {
    pvac_cipher a = pvac_deserialize_cipher(a_data, (size_t)a_len);
    pvac_cipher b = pvac_deserialize_cipher(b_data, (size_t)b_len);
    ...
    pvac_cipher result = pvac_ct_add(g_pk, a, b);
    pvac_free_cipher(a);
    pvac_free_cipher(b);
    size_t len = 0;
    uint8_t* data = pvac_serialize_cipher(result, &len);
...
// Multiply two ciphers homomorphically (seeded, deterministic)
EMSCRIPTEN_KEEPALIVE
uint8_t* pvac_wasm_ct_mul_seeded(const uint8_t* a_data, int a_len,
                                  const uint8_t* b_data, int b_len,
                                  const uint8_t* seed,
                                  int* out_len) {
    pvac_cipher a = pvac_deserialize_cipher(a_data, (size_t)a_len);
    ...
    pvac_cipher result = pvac_ct_mul_seeded(g_pk, a, b, seed);
    pvac_free_cipher(a);
    pvac_free_cipher(b);
    size_t len = 0;
    uint8_t* data = pvac_serialize_cipher(result, &len);
...
// Add a constant to a cipher
EMSCRIPTEN_KEEPALIVE
uint8_t* pvac_wasm_ct_add_const(const uint8_t* ct_data, int ct_len,
                                 uint64_t k_lo, uint64_t k_hi,
                                 int* out_len) {
    pvac_cipher ct = pvac_deserialize_cipher(ct_data, (size_t)ct_len);
    if (!ct) { *out_len = 0; return nullptr; }
    pvac_cipher result = pvac_ct_add_const(g_pk, ct, k_lo, k_hi);
    pvac_free_cipher(ct);
    size_t len = 0;
    uint8_t* data = pvac_serialize_cipher(result, &len);
    pvac_free_cipher(result);
