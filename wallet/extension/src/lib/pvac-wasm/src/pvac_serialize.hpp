/**
 * PVAC binary serialization for ciphers, keys, and proofs.
 * FILE: src/pvac_serialize.hpp
 * STATUS: OUR CODE (not vendored)
 *
 * Handles compact binary encoding/decoding of PVAC types for transport
 * between WASM and the Octra node. Format is versioned (PVAC magic + v2).
 */
#pragma once
