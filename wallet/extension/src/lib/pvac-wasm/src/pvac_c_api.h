/**
 * PVAC C API — opaque-handle interface to the PVAC C++ library.
 * FILE: src/pvac_c_api.h
 * STATUS: OUR CODE (not vendored)
 *
 * Provides a C-linkage API wrapping PVAC's C++ types (Cipher, PubKey, SecKey,
 * ZeroProof, RangeProof, etc.) behind opaque void* handles.
 */
#pragma once
