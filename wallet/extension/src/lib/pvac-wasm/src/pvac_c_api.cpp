/**
 * PVAC C API implementation.
 * FILE: src/pvac_c_api.cpp
 * STATUS: OUR CODE (not vendored)
 *
 * Implements the C-linkage API defined in pvac_c_api.h by calling into the
 * vendored PVAC C++ headers (vendor/pvac/).
 */
...
pvac_zero_proof pvac_make_zero_proof_bound(pvac_pubkey pk, pvac_seckey sk, pvac_cipher ct,
    ...
    try {
        ...
        size_t nL = CT(ct)->L.size();
        size_t S = CT(ct)->slots;
        
        // Step 1: cache layer R values
        std::vector<std::vector<pvac::Fp>> cache(nL);
        std::vector<uint8_t> st(nL, 0);
        for (size_t lid = 0; lid < nL; lid++)
            pvac::layer_R_cached(*PK(pk), *SK(sk), *CT(ct), (uint32_t)lid, st, cache);
        
        // Step 2: compute rinv
        std::vector<std::vector<pvac::Fp>> rinv(nL);
        for (size_t lid = 0; lid < nL; lid++) {
            rinv[lid].resize(S);
            for (size_t j = 0; j < S; j++)
                rinv[lid][j] = pvac::fp_inv(cache[lid][j]);
        }
        
        // Step 3: compute layer coefficients
        auto A = pvac::compute_layer_coeffs(*PK(pk), *CT(ct));
        
        // Step 4: base layer indices
        auto bases = pvac::base_layer_indices(*CT(ct));
        
        // Step 5: build circuit
        pvac::detail::AmountBinding bind;
        bind.amount = amount;
        bind.blinding = blind;
        
        pvac::bp::R1CSProver prover;
        auto wiring = pvac::detail::build_circuit(prover, *CT(ct), A, bases, &rinv, SK(sk), &bind);
        
        // Step 6: prove
        pvac::bp::Transcript transcript("pvac.verify_zero.circuit");
        transcript.append_u64("nL", nL);
        transcript.append_u64("S", S);
        transcript.append_u64("nB", bases.size());
        
        auto* zp = new pvac::ZeroProof();
        zp->proof = prover.prove(transcript);
        zp->is_bound = true;
        return zp;
    } catch (...) {
        return nullptr;
