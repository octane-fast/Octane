...
export function ctAdd(a: Uint8Array, b: Uint8Array): Uint8Array {
  if (!initialized) throw new Error('PVAC not initialized');
  const aPtr = module._malloc(a.length);
  module.HEAPU8.set(a, aPtr);
  const bPtr = module._malloc(b.length);
  ...
  const dataPtr = module._pvac_wasm_ct_add(aPtr, a.length, bPtr, b.length, outLenPtr);
  const outLen = module.getValue(outLenPtr, 'i32');
  const result = dataPtr ? new Uint8Array(module.HEAPU8.buffer, dataPtr, outLen).slice() : new Uint8Array(0);
...
export function ctMul(a: Uint8Array, b: Uint8Array, seed: Uint8Array): Uint8Array {
  if (!initialized) throw new Error('PVAC not initialized');
  ...
  module.HEAPU8.set(b, bPtr);
  const seedPtr = module._malloc(32);
  module.HEAPU8.set(seed.subarray(0, 32), seedPtr);
  const outLenPtr = module._malloc(4);

  const dataPtr = module._pvac_wasm_ct_mul_seeded(aPtr, a.length, bPtr, b.length, seedPtr, outLenPtr);
  const outLen = module.getValue(outLenPtr, 'i32');
  const result = dataPtr ? new Uint8Array(module.HEAPU8.buffer, dataPtr, outLen).slice() : new Uint8Array(0);
...
export function ctAddConst(ct: Uint8Array, k: bigint): Uint8Array {
  if (!initialized) throw new Error('PVAC not initialized');
  const ctPtr = module._malloc(ct.length);
  module.HEAPU8.set(ct, ctPtr);
  const outLenPtr = module._malloc(4);

  const kLo = Number(k & 0xFFFFFFFFFFFFFFFFn);
  const kHi = Number((k >> 64n) & 0xFFFFFFFFFFFFFFFFn);

  const dataPtr = module._pvac_wasm_ct_add_const(ctPtr, ct.length, kLo, kHi, outLenPtr);
  const outLen = module.getValue(outLenPtr, 'i32');
  const result = dataPtr ? new Uint8Array(module.HEAPU8.buffer, dataPtr, outLen).slice() : new Uint8Array(0);
...
export function ctSubConst(ct: Uint8Array, k: bigint): Uint8Array {
  return ctAddConst(ct, -k);
}
